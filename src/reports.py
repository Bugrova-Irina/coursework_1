...
def get_spending_by_categories(transactions: pd.DataFrame, category: str, end_date: Optional[str] = None) -> pd.DataFrame:
    """
    ...
    try:
        ...
        # преобразуем дату операции в строку для сериализации
        # filtered_transactions["Дата операции"] = filtered_transactions["Дата операции"].df.strftime('%d.%m.%Y %H:%M:%S')
...
def get_json_file(func):
    """
    Записывает полученный ранее отчет в формате датафрейма в json-файл
    """
    def wrapper(*args, **kwargs):
        result = func(*args, **kwargs)

        if isinstance(result, pd.DataFrame):
            wrapper_result = result.to_dict("records") # преобразуем датафрейм в список словарей
...
if __name__ == "__main__":
    ...
    # print(type(get_spending_by_categories(get_transactions_xlsx(), "Супермаркеты", "30.12.2021 19:06:39")))
    transactions_df = get_transactions_xlsx()
    # print(transactions_df)

    report = generate_report(transactions_df, "Супермаркеты", "30.12.2021 19:06:39")
    print(f"Стандартный отчет записан в report.json")

    my_report = generate_my_report(transactions_df, "Супермаркеты", "30.12.2021 19:06:39")
    print(f"Отчет записан в выбранный пользователем файл my_report.json")
